...
export function TransactionHistory({ apiKey }: TransactionHistoryProps) {
  ...
  const getTypeIcon = (type: string) => {
    switch (type) {
      case "BUY":
        return <ArrowUpIcon className="h-4 w-4 text-[#00e0b8]" />
      case "SELL":
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case "WIN":
        return <Badge className="bg-[#00e0b8]/10 text-[#00e0b8] border-[#00e0b8]/30">Ganhou</Badge>
      case "LOSS":
  ...
  const getResultColor = (result: number) => {
    if (result > 0) return "text-[#00e0b8]"
